...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item= None):
        ...
        # get grid lines for token boxes (i.e., pixel coordinates for token location on the image)
        ...
        if self.train:
            return images, word_vec, torch.LongTensor([len(wrds)+2]), torch.LongTensor(dim_mat)
        else:
